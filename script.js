        ...
        contactForm.addEventListener('submit', function(e) {
            ...
            fields.forEach(function(field) {
                ...
                if (field.hasAttribute('required') && value === '') {
                    errorMsg = field.getAttribute('data-error-required') || 'هذا الحقل مطلوب';
                }
                if (!errorMsg && field.type === 'email' && value !== '') {
                    var emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
                    if (!emailRegex.test(value)) {
                        errorMsg = field.getAttribute('data-error-email') || 'يرجى إدخال بريد إلكتروني صالح';
                    }
                }
                if (!errorMsg && field.hasAttribute('minlength') && value.length < parseInt(field.getAttribute('minlength'))) {
                    errorMsg = field.getAttribute('data-error-minlength') || 'الحد الأدنى ' + field.getAttribute('minlength') + ' أحرف';
                }
                if (!errorMsg && field.hasAttribute('pattern') && value !== '') {
                    var pattern = new RegExp(field.getAttribute('pattern'));
                    if (!pattern.test(value)) {
                        errorMsg = field.getAttribute('data-error-pattern') || 'تنسيق غير صالح';
                    }
                }
            ...
            if (isValid) {
                ...
                if (submitBtn) {
                    ...
                    submitBtn.textContent = submitBtn.getAttribute('data-loading') || 'جاري الإرسال...';
                }
                contactForm.submit();
            } else if (firstError) {
                firstError.focus();
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: '33ff94d3-1c67-4547-8329-70e951b81982',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || '33ff94d3-1c67-4547-8329-70e951b81982';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '33ff94d3-1c67-4547-8329-70e951b81982';
